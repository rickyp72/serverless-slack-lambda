...
def cloudwatch_notification(message, region):
    ...
    return {
        ...
        "fields": [
            ...
            {
                "title": "Link to Alarm",
                "value": "https://console.aws.amazon.com/cloudwatch/home?region=" + region + "#alarm:alarmFilter=ANY;name=" + urllib.parse.quote(
                    message['AlarmName']),
                "short": False
            }
        ]
...
def default_notification(subject, message):
    return {
        ...
        "fields": [{"title": subject if subject else "Message",
                    "value": json.dumps(message) if type(message) is dict else message, "short": False}]
    }
...
def notify_slack(subject, message, region):
    ...
    if slack_url.startswith("http"):
        req = urllib.request.Request(slack_url)
    else:
        slack_url = f"https://{slack_url}"
        req = urllib.request.Request(slack_url)
...
def lambda_handler(event, context):
    ...
    if json.loads(response)["code"] != 200:
        logging.error(
            "Error: received status `{}` using event `{}` and context `{}`".format(json.loads(response)["info"], event,
                                                                                   context))
